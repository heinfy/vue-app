export const checkIdCard = function(id) {
  ...
  if (reg.test(id)) {
    let checkProvince = id.substr(0, 2)
    if (checkProvince in provinces) {
      let coefficient = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2]
      let coeOfValue = ['1', '0', 'X', '9', '8', '7', '6', '5', '4', '3', '2']
      let idCardArray = id.split('')
      let idCardCeoSum = 0
      for (let i = 0; i < 17; i++) {
        idCardCeoSum += idCardArray[i] * coefficient[i]
      }
      let lastValue = coeOfValue[idCardCeoSum % 11]
      return lastValue.toLowerCase() === id.substr(17, 1).toLowerCase() ? true : false
    } else {
      return false
    }
  } else {
